...
const MIN_ZH_CHARACTERS = 200;
const MIN_EN_WORDS = 130;
...
function nonEmptyString(
  value,
  path,
  { maxLength = Number.POSITIVE_INFINITY, truncate = false } = {},
) {
...
function localizedString(
  ...
  if (!value || typeof value !== "object" || Array.isArray(value))
    fail(path, "must be an object with zh and en");
  return {
...
function localizedTags(value, path) {
  if (!value || typeof value !== "object" || Array.isArray(value))
    fail(path, "must be an object with zh and en arrays");
  const result = {};
  for (const locale of LOCALES) {
    if (!Array.isArray(value[locale]))
      fail(path + "." + locale, "must be an array");
    const tags = value[locale].map((tag, index) =>
      nonEmptyString(tag, path + "." + locale + "[" + index + "]", {
        maxLength: 40,
      }),
    );
    const unique = [...new Set(tags)];
    if (unique.length < 1 || unique.length > 6)
      fail(path + "." + locale, "must contain 1 to 6 unique tags");
...
function validateBlocks(value, path) {
  ...
  if (value.length < 3 || value.length > MAX_BLOCKS)
    fail(path, "must contain 3 to " + MAX_BLOCKS + " blocks");

  let headingCount = 0;
  const blocks = value.map((block, index) => {
    const blockPath = path + "[" + index + "]";
    if (!block || typeof block !== "object" || Array.isArray(block))
      fail(blockPath, "must be an object");
    if (
      block.type === "heading" ||
      ...
      return {
        type: block.type,
        text: localizedString(block.text, blockPath + ".text"),
      };
    }
    if (block.type === "code") {
      return {
...
function textFromBlock(block, locale) {
  ...
  if (block.text && typeof block.text === "object") {
    return String(
      block.text[locale] ?? block.text.zh ?? block.text.en ?? "",
    ).trim();
  }
...
function coerceContentBlocks(value, fallbackTitle, locale) {
  ...
  if (value && typeof value === "object") {
    if (Array.isArray(value[locale]))
      return coerceContentBlocks(value[locale], fallbackTitle, locale);
    if (
      value.type ||
    ...
    for (const key of ["blocks", "sections", "paragraphs"]) {
      if (Array.isArray(value[key])) {
        ...
        return value.title
          ? [{ type: "heading", text: value.title }, ...blocks]
          : blocks;
      }
    }
...
function normalizeContent(draftContent, title) {
  ...
  if (isNativeBlockArray(raw.zh) && isNativeBlockArray(raw.en))
    return { zh, en };
  ...
  const paired = Array.from({ length }, (_, index) => {
    ...
    if (zhType === "code" || enType === "code") {
      ...
      return {
        type: "code",
        language:
          typeof codeBlock.language === "string" && codeBlock.language.trim()
            ? codeBlock.language
            : "text",
        code:
          typeof codeBlock.code === "string"
            ? codeBlock.code
            : textFromBlock(codeBlock, "en"),
      };
    }
    const type =
      zhType === "heading" || enType === "heading"
        ? "heading"
        : zhType === "quote" || enType === "quote"
          ? "quote"
          : "paragraph";
    return {
      type,
...
function validateContentLength(content) {
  for (const locale of LOCALES) {
    const paragraphs = content[locale].filter(
      (block) => block.type === "paragraph",
    ).length;
    if (paragraphs < MIN_PARAGRAPHS) {
      fail(
        "content." + locale,
        "must contain at least " + MIN_PARAGRAPHS + " paragraph blocks",
      );
    }
  }

  const zhCharacters = [...countLocaleText(content.zh, "zh")].filter(
    (char) => char >= "一" && char <= "鿿",
  ).length;
  if (zhCharacters < MIN_ZH_CHARACTERS) {
    fail(
      "content.zh",
      "must contain at least " + MIN_ZH_CHARACTERS + " Chinese characters",
    );
  }
  ...
  if (enWords < MIN_EN_WORDS) {
    fail(
      "content.en",
      "must contain at least " + MIN_EN_WORDS + " English words",
    );
  }
}

function calculateReadingTime(content) {
  const text = content.zh
    .concat(content.en)
    .map((block) =>
      block.type === "code" ? block.code : block.text.zh + " " + block.text.en,
    )
    .join(" ");
  const chineseCharacters = [...text].filter(
    (char) => char >= "一" && char <= "鿿",
  ).length;
  ...
  return Math.min(
    60,
    Math.max(1, Math.ceil(Math.max(chineseCharacters / 450, latinWords / 220))),
  );
}
...
export function validateGeneratedDraft(draft) {
  ...
  const title = localizedString(draft.title, "title", {
    maxLength: MAX_TITLE_LENGTH,
    truncate: true,
  });
  const excerpt = localizedString(draft.excerpt, "excerpt", {
    maxLength: MAX_EXCERPT_LENGTH,
    truncate: true,
  });
  const category = localizedString(draft.category, "category", {
    maxLength: 80,
  });
  const tags = localizedTags(draft.tags, "tags");
  if (draft.slug !== undefined && typeof draft.slug !== "string")
    fail("slug", "must be a string when provided");
...
export function createGeneratedArticleDocument(
  story,
  draft,
  {
    model,
  ...
  if (
    !story ||
    typeof story.originalUrl !== "string" ||
    !story.originalUrl.trim()
  ) {
    fail("source.originalUrl", "must be a non-empty string");
  }
  if (!model || typeof model !== "string")
    fail("generation.model", "must be a non-empty string");

  // 有 HN 讨论时携带 HN 元数据；指定链接模式没有 HN 来源，仅保留原文信息
  const hnId =
    Number.isInteger(story.hnId) && story.hnId > 0 ? story.hnId : null;
  ...
  const slug =
    normalized.slug +
    (hnId ? "-hn-" + hnId : "-web-" + urlHashHex(story.originalUrl));

  return {
    schemaVersion: 1,
    source: {
      hnId,
      hnUrl: hnId ? story.hnUrl : null,
    ...
    post: {
      ...
      disclosure: {
        ...(hnId ? WEEKLY_DAY_AI.disclosure : WEEKLY_DAY_AI.disclosureWeb),
      },
