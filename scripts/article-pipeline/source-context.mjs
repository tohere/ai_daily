...
function extractHtmlTitle(html) {
  ...
  const tagMatch = !ogMatch && html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
  const raw = (ogMatch?.[1] ?? tagMatch?.[1] ?? "").trim();
  if (!raw) return "";
  return htmlToPlainText(raw, { maxCharacters: MAX_TITLE_CHARACTERS })
    .replace(/\s+/g, " ")
    .trim();
}

// 一次抓取同时返回标题与正文（指定链接模式使用）
export async function fetchOriginalArticle(
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      return {
        ...
        text: htmlToPlainText(html, { maxCharacters }),
      };
    } catch (error) {
      lastError = error;
  ...
  onWarning(
    "Unable to read original article " +
      normalizedUrl +
      ": " +
...
export function htmlToPlainText(html, { maxCharacters = 12000 } = {}) {
  ...
  return decodeHtmlEntities(
    html
      ...
      .replace(/<(br|hr)\b[^>]*>/gi, "\n")
      .replace(
        /<\/(p|div|section|article|main|header|footer|aside|nav|li|h[1-6]|blockquote|pre)>/gi,
        "\n",
      )
      .replace(/<[^>]+>/g, " "),
  )
...
export async function fetchOriginalArticleText(
  url,
  {
    fetchImpl = globalThis.fetch,
    retries = 2,
