    ...
    ...Array.from({ length: 6 }, (_, index) => ({
      ...
      text: {
        zh: (
          "这是关于人工智能系统的第" +
          (index + 1) +
          "段测试内容，介绍背景、技术机制、实际影响、局限性以及后续值得继续观察的问题。"
        ).repeat(4),
        en: "This paragraph explains the background, technical mechanism, practical impact, limitations, and open questions of the AI system. ".repeat(
          12,
        ),
      },
    })),
...
// Responses API 非流式响应形状
const responsesJson = (text) => ({
  status: "completed",
...
test("loadAiConfig normalizes endpoint and reads numeric options", () => {
  ...
  assert.equal(config.endpoint, "https://gateway.example.com/v1/responses");
  assert.equal(config.temperature, 0.7);
  assert.equal(config.maxTokens, 2000);
  assert.equal(config.requestRetries, 1);
});

test("requestArticleDraft sends a Responses API request and parses fenced JSON", async () => {
  let requestUrl;
  let requestOptions;
  const fetchImpl = async (url, options) => {
    ...
    return response(
      responsesJson(
        fence + "json" + newline + JSON.stringify(draft) + newline + fence,
      ),
    );
  };
  const config = loadAiConfig(configEnv);
  const result = await requestArticleDraft(story, "<p>source</p>", {
    config,
    fetchImpl,
    retryDelayMs: 0,
  });
  ...
  assert.match(body.input[1].content, /A test story/);
  assert.doesNotMatch(body.input[1].content, /secret-key/);
...
test("validateGeneratedDraft normalizes localized text content into blocks", () => {
    ...
    content: {
      zh: Array.from(
        ...
        (_, index) =>
          "第" +
          (index + 1) +
          "段人工智能分析内容，介绍背景、技术机制、实际影响、局限性和未来问题。".repeat(
            8,
          ),
      ).join("\n\n"),
      en: Array.from(
        ...
        (_, index) =>
          "Paragraph " +
          (index + 1) +
          " explains the background, technical mechanism, practical impact, limitations, and open questions for this AI story. ".repeat(
            18,
          ),
      ).join("\n\n"),
    },
  ...
  assert.ok(
    normalized.content.zh.filter((block) => block.type === "paragraph")
      .length >= 6,
  );
  assert.ok(
    normalized.content.en.filter((block) => block.type === "paragraph")
      .length >= 6,
  );
});
...
test("validateGeneratedDraft repairs missing English text in a bilingual native block", () => {
  const brokenContent = {
    zh: longContent.zh.map((block, index) =>
      index === 1 ? { ...block, text: { ...block.text, en: String() } } : block,
    ),
    en: longContent.en,
  };
  ...
  assert.equal(
    normalized.content.zh[1].text.en.trim(),
    longContent.en[1].text.en.trim(),
  );
  assert.equal(
    normalized.content.en[1].text.en.trim(),
    longContent.en[1].text.en.trim(),
  );
});
...
test("validateGeneratedDraft normalizes object-shaped localized content", () => {
  const normalized = validateGeneratedDraft({
    ...
    content: {
      ...
      en: {
        introduction:
          "This introduction explains the background and the central question. ".repeat(
            30,
          ),
        facts:
          "This section separates reported facts from cautious interpretation. ".repeat(
            30,
          ),
        technical:
          "This section explains the technical mechanism and its practical meaning. ".repeat(
            30,
          ),
        impact:
          "This section discusses the implications for users, teams, and products. ".repeat(
            30,
          ),
        limits:
          "This section lists limitations and questions that still require verification. ".repeat(
            30,
          ),
        conclusion:
          "This conclusion summarizes the main points and future directions. ".repeat(
            30,
          ),
      },
    },
  ...
  assert.ok(Array.isArray(normalized.content.en));
  assert.equal(
    normalized.content.en.filter((block) => block.type === "paragraph").length,
    6,
  );
});
...
test("requestArticleDraft rejects invalid JSON", async () => {
  ...
  await assert.rejects(
    requestArticleDraft(story, "", {
      config,
      fetchImpl: async () => response(responsesJson("not json")),
      retryDelayMs: 0,
    }),
    /not valid JSON/,
  );
...
test("generated document uses fixed disclosure, provider link, and HN id slug suffix", async () => {
  ...
  const document = await generateArticleDocument(story, {
    ...
    fetchImpl: async () => response(responsesJson(JSON.stringify(draft))),
...
test("htmlToPlainText removes active content and decodes entities", () => {
  const result = htmlToPlainText(
    "<style>bad</style><p>Hello &amp; world</p><script>bad()</script><p>Next</p>",
  );
...
test("writeGeneratedArticle uses exclusive creation and refuses overwrite", async () => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "ai-daily-generated-"),
  );
  try {
    const config = loadAiConfig(configEnv);
    const document = await generateArticleDocument(story, {
    ...
    assert.equal(
      JSON.parse(await readFile(filePath, "utf8")).source.hnId,
      12345,
    );
    await assert.rejects(
      writeGeneratedArticle(document, directory),
