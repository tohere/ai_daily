...
export async function generateArticleDocument(
  ...
  const resolvedSourceText =
    sourceText === undefined
      ? await fetchOriginalArticleText(story.originalUrl, {
          fetchImpl: sourceFetchImpl,
          ...sourceOptions,
        })
      : sourceText;
  const draft = await requestArticleDraft(story, resolvedSourceText, {
    config,
    fetchImpl,
    ...aiOptions,
  });
...
export function buildGeneratedPostFilename(document) {
  ...
  const key =
    Number.isInteger(hnId) && hnId > 0
      ? "hn-" + hnId
      : "web-" + urlHashHex(document.source.originalUrl);
  return document.post.date + "-" + key + ".json";
}

export async function writeGeneratedArticle(document, directory) {
  if (!document?.source?.originalUrl || !document?.post?.date)
    throw new Error("A complete generated article document is required");
