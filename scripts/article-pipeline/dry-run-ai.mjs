...
const draft = {
  ...
  title: {
    zh: "构建可靠 AI 工作流的实用方法",
    en: "Practical ways to build reliable AI workflows",
  },
  excerpt: {
    zh: "从输入整理、事实边界和人工复核三个方面，理解如何让 AI 工作流更可靠。",
    en: "How organized inputs, factual boundaries, and human review make AI workflows more reliable.",
  },
  category: { zh: "人工智能", en: "Artificial Intelligence" },
  tags: {
    zh: ["AI", "工作流", "可靠性"],
    en: ["AI", "Workflow", "Reliability"],
  },
  content: {
    zh: [
      {
        ...
        text: {
          zh: "为什么可靠性比速度更重要",
          en: "Why reliability matters more than speed",
        },
      },
      {
        type: "paragraph",
        text: {
          zh: "在真实的 AI 应用中，可靠性往往比响应速度更影响用户的长期信任。一个能够稳定给出可核查结果的系统，即使响应稍慢一些，也更容易被引入生产环境。团队在评估模型时，除了关注基准测试的分数，还应该观察它的失败模式是否可预测、错误的输出是否容易被发现和纠正。毕竟线上环境无法容忍随机出现的幻觉内容。",
          en: "In real-world AI applications, reliability usually matters more than raw speed when it comes to earning long-term user trust. A system that consistently produces verifiable results is easier to promote into production, even if it responds a little more slowly. When teams evaluate models, they should look beyond benchmark scores and study whether failures are predictable and whether bad outputs can be detected and corrected quickly, because live environments cannot tolerate randomly appearing hallucinations.",
        },
      },
      {
        type: "heading",
      ...
      },
      {
        type: "paragraph",
        text: {
          zh: "总而言之，可靠性是一项持续的系统工程，而不是一次性的配置任务。随着模型版本升级和数据分布变化，原本稳定的流程也可能悄悄退化。定期回顾失败案例、更新约束条件、调整验收标准，才能让系统在长期运行中保持可信。这些做法并不复杂，却往往决定了一个 AI 应用能否真正交付价值。",
          en: "In short, reliability is continuous engineering rather than a one-time configuration task. As models are upgraded and data distributions shift, a process that used to be stable can quietly degrade over time. Reviewing failure cases regularly, refreshing constraints, and adjusting acceptance criteria are what keep a system trustworthy across long-running operations. None of these practices is complicated on its own, yet together they often decide whether an AI application can actually deliver lasting value to its readers.",
        },
      },
    ],
    en: [
...
const sourceFetchImpl = async () => ({
  ...
  text: async () =>
    "<html><head><style>hidden</style></head><body><h1>Source</h1><p>Reliable AI needs explicit constraints.</p><script>alert(1)</script></body></html>",
});
