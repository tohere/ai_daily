...
export async function fetchJsonWithRetry(
  ...
  throw new Error(
    `Failed to fetch ${url} after ${retries + 1} attempt(s): ${lastError?.message ?? lastError}`,
  );
}
...
export async function fetchTopStoryIds({
  ...
  if (!Array.isArray(result))
    throw new Error("Hacker News topstories response must be an array");
  return result.filter((id) => Number.isInteger(id) && id > 0).slice(0, limit);
}

export async function fetchHackerNewsItem(
  id,
  { fetchImpl, retries, timeoutMs } = {},
) {
  if (!Number.isInteger(id) || id <= 0)
    throw new Error(`Invalid Hacker News item id: ${id}`);
  return fetchJsonWithRetry(`${HN_API_BASE_URL}/item/${id}.json`, {
    fetchImpl,
    retries,
    timeoutMs,
  });
...
// 关键词搜索模式：通过 HN Algolia 按相关度选题（不受热门榜分数/评论门槛限制）
export async function searchHackerNewsStories({
  ...
  for (const hit of hits) {
    ...
    if (
      !Number.isInteger(hnId) ||
      hnId <= 0 ||
      seen.has(hnId) ||
      publishedHnIds.has(hnId)
    )
      continue;

    const originalUrl =
      parseExternalUrl(hit?.url) ?? parseExternalUrl(hit?.story_url);
    ...
    const publishedAt =
      ...
      !Number.isNaN(Date.parse(hit.created_at))
    ...
    selected.push({
      hnId,
      hnUrl: HN_ITEM_URL_PREFIX + hnId,
      originalUrl,
      title,
      author:
        typeof hit?.author === "string" && hit.author.trim()
          ? hit.author.trim()
          : "unknown",
      score: Math.max(
        0,
        Number.isFinite(hit.points) ? Math.trunc(hit.points) : 0,
      ),
      commentCount: Math.max(
        0,
        Number.isFinite(hit.num_comments) ? Math.trunc(hit.num_comments) : 0,
      ),
      publishedAt,
    });
  ...
  if (selected.length < count) {
    throw new Error(
      `Only ${selected.length} search result(s) with valid links found for topic "${topic.trim()}"; ${count} required. Try broader keywords.`,
    );
  }
...
export function normalizeHackerNewsStory(item) {
  ...
  return {
    ...
    score: Math.max(
      0,
      Number.isFinite(item.score) ? Math.trunc(item.score) : 0,
    ),
    commentCount: Math.max(
      0,
      Number.isFinite(item.descendants) ? Math.trunc(item.descendants) : 0,
    ),
...
export function calculateStoryRank(story, now = new Date()) {
  const ageHours = Math.max(
    0,
    (now.getTime() - Date.parse(story.publishedAt)) / 3_600_000,
  );
  const freshnessBonus = Math.max(0, 72 - ageHours) * 2;
  return Number(
    (story.score + story.commentCount * 1.5 + freshnessBonus).toFixed(3),
  );
}
...
export async function selectHackerNewsStories({
  ...
  const items = await mapWithConcurrency(ids, fetchConcurrency, async (id) => {
    try {
      return await fetchHackerNewsItem(id, {
        fetchImpl,
        retries: requestRetries,
        timeoutMs: requestTimeoutMs,
      });
  ...
  const topicMatched = normalized.filter((story) =>
    isAiRelatedStory(story, aiKeywords),
  );
  const eligible = topicMatched
    ...
    .sort(
      (left, right) =>
        ...
        right.hnId - left.hnId,
    );

  const selected = eligible.slice(0, count);
  if (selected.length < count) {
    throw new Error(
      `Only ${selected.length} eligible Hacker News stories found; ${count} required. Fetched ${ids.length} ids, normalized ${normalized.length}, and skipped ${publishedHnIds.size} published id(s).`,
    );
  }
